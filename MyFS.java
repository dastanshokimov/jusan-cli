...
public class MyFS {
    ...
    // Для вывода всех файлов и директорий для `path` - ls
    ...
    // Для вывода файлов с расширением `.py` в `path` - ls_py
    ...
    // Для вывода `true`, если `path` это директория, в других случаях `false` - id_dir
    ...
    // Выводит `директорию` или `файл` в зависимости от типа `path` - define
    ...
    // Для вывода права для файла в формате `rwx` для текущего пользователя - readmod
    ...
    // Создает копию `path` в директорию `/tmp/${date}.backup` где, date - это дата в формате `dd-mm-yyyy`. `path` может быть директорией или файлом.
    // При директории, копируется весь контент. - bc
